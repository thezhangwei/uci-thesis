...
After an integer overflow, ZipPy uses a JDK \texttt{BigInteger}~\ref{hotspot} to model integers of a large value.
...
For instance, in some versions of the HotSpot VM~\cite{hotspot}, the JVM uses a technique called pointer compression to reduce the size of a Java object pointer inside the VM.
...
The the overall performance evaluation, the space configuration of the fixed object storage in ZipPy is five.
...
Flexible object storage with continuous generation means we always generate a new storage class whenever a layout change takes place.

\subsection{The Performance of Flexible Object Storages}

\begin{figure}
\centering
\includegraphics[scale=.35, page=4]{benchmarks/object-model-results}
\caption{Detailed speedups of different object model configurations normalized to fixed object storage}
...
\caption{Fixed object storage memory overhead with slot size $1$, $3$ and $5$ relative to flexible storage allocation with continuous generation}
...
\caption{Fixed object storage slowdowns with slot size $1$, $3$ and $5$ relative to flexible storage allocation with continuous generation}
